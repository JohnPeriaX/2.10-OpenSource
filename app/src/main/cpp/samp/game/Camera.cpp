...
void CCamera::Init() {
    CHook::CallFunction<void>(g_libGTASA + 0x55BA30, this);
}

void CCamera::SetRwCamera(RwCamera *pCamera) {
    CHook::CallFunction<void>(g_libGTASA + 0x4BF318, this, pCamera);
}

void CCamera::TakeControl(CEntityGTA *target, eCamMode modeToGoTo, eSwitchType switchType, int32 whoIsInControlOfTheCamera) {
    CHook::CallFunction<void>(g_libGTASA + 0x4BF474, this, target, modeToGoTo, switchType, whoIsInControlOfTheCamera);
}

float CCamera::CalculateGroundHeight(eGroundHeightType type) {
    return CHook::CallFunction<float>(g_libGTASA + 0x4BA958, this, type);
}

void CCamera::RestoreWithJumpCut() {
    CHook::CallFunction<void>(g_libGTASA + 0x4B94B4, this);
}

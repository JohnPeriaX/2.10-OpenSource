...
void CPedGTA::GetBonePosition(RwV3d *posn, uint32 boneTag, bool bCalledFromCamera) {
    CHook::CallFunction<void>(g_libGTASA + 0x59AEE4, this, posn, boneTag, bCalledFromCamera);
}
...
CPedGTA::~CPedGTA() {
    CHook::CallFunction<void>(g_libGTASA + 0x59541C, this);
}
...
void CPedGTA::GiveWeapon(int iWeaponID, int iAmmo)
{
    ...
    CHook::CallFunction<void>(g_libGTASA + 0x59525C, this, iWeaponID, iAmmo); // CPed::GiveWeapon(thisptr, weapoid, ammo)
    CHook::CallFunction<void>(g_libGTASA + 0x59B86C, this, iWeaponID);	// CPed::SetCurrentWeapon(thisptr, weapid)
}

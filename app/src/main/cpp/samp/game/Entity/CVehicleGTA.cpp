...
void CVehicleGTA::RenderDriverAndPassengers() {
    ...
    if (pDriver && pDriver->m_nPedState == PEDSTATE_DRIVING) {
        CHook::CallFunction<void>(g_libGTASA + 0x59D3B8, pDriver);
        // pDriver->Render();
    }

    for (auto& passenger : m_apPassengers) {
        if (passenger && passenger->m_nPedState == PEDSTATE_DRIVING) {
            CHook::CallFunction<void>(g_libGTASA + 0x59D3B8, passenger);
            // passenger->Render();
        }
    }
}
...
void CVehicleGTA::AddVehicleUpgrade(int32 modelId) {
    CHook::CallFunction<void>(g_libGTASA + 0x6AFF4C, this, modelId);
}

void CVehicleGTA::RemoveVehicleUpgrade(int32 upgradeModelIndex) {
    CHook::CallFunction<void>(g_libGTASA + 0x6B0718, this, upgradeModelIndex);
}
...
void CVehicleGTA::InjectHooks() {
    // var
    CHook::Write(g_libGTASA + 0x849EA8, &CVehicleGTA::m_aSpecialColModel);

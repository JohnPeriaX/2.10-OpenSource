...
CSector* GetSector(int32 x, int32 y) {
    static CSector(&ms_aSectors)[MAX_SECTORS_Y][MAX_SECTORS_X] = *(CSector(*)[MAX_SECTORS_Y][MAX_SECTORS_X])(g_libGTASA + 0xBDCB20);
...
CRepeatSector* GetRepeatSector(int32 x, int32 y) {
    static CRepeatSector(&ms_aRepeatSectors)[MAX_REPEAT_SECTORS_Y][MAX_REPEAT_SECTORS_X] = *(CRepeatSector(*)[MAX_REPEAT_SECTORS_Y][MAX_REPEAT_SECTORS_X])(g_libGTASA + 0xC14F20);
...
void CWorld::InjectHooks() {
    ...
    CHook::InstallPLT(g_libGTASA + 0x849A20, &ProcessPedsAfterPreRender);
}
...
bool CWorld::ProcessLineOfSight(const CVector* origin, const CVector* target, CColPoint* outColPoint, CEntityGTA** outEntity, bool buildings, bool vehicles, bool peds, bool objects, bool dummies, bool doSeeThroughCheck, bool doCameraIgnoreCheck, bool doShootThroughCheck) {
   // assert(!origin.HasNanOrInf() && !target.HasNanOrInf()); // We're getting random nan/inf's from somewhere, so let's try to root cause it...
    return CHook::CallFunction<bool>(g_libGTASA + 0x508C7C, origin, target, outColPoint, outEntity, buildings, vehicles, peds, objects, dummies, doSeeThroughCheck, doCameraIgnoreCheck, doShootThroughCheck);
}
float CWorld::FindGroundZForCoord(float x, float y) {
    return CHook::CallFunction<float>(g_libGTASA + 0x50F3A0, x, y);
}
...
bool CWorld::GetIsLineOfSightClear(const CVector& origin, const CVector& target, bool buildings, bool vehicles, bool peds, bool objects, bool dummies, bool doSeeThroughCheck, bool doCameraIgnoreCheck) {
    return CHook::CallFunction<bool>(g_libGTASA + 0x5075A4, &origin, &target, buildings, vehicles, peds, objects, dummies, doSeeThroughCheck, doCameraIgnoreCheck);
}

void CWorld::Add(CEntityGTA *entity) {
    ((void(*)(CEntityGTA*))(g_libGTASA + 0x507518))(entity);
}

void CWorld::Remove(CEntityGTA *entity) {
    ((void (*)(CEntityGTA*))(g_libGTASA + 0x5073A0)) (entity);
}

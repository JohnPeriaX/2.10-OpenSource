...
int32_t NVEventGetNextEvent_hook(NVEvent* ev, int waitMSecs)
{
    ...
    if(NVEventGetNextEvent(&event))
    {
        ...
        if ((x1 || y1) || num == 0)
        {
            ...
            {
                ((void(*)(int, int, int posX, int posY))(g_libGTASA + 0x31EC0C))(type, 0, x1, y1); // AND_TouchEvent
            }
            else
            {
                ((void(*)(int, int, int posX, int posY))(g_libGTASA +  0x31EC0C))(NV_MULTITOUCH_MOVE, 0, x1, y1); // AND_TouchEvent
            }
        }
        ...
        if ((x2 || y2) || num == 1)
        {
            ...
            {
                ((void(*)(int, int, int posX, int posY))(g_libGTASA +  0x31EC0C))(type, 1, x2, y2); // AND_TouchEvent
            }
            else
            {
                ((void(*)(int, int, int posX, int posY))(g_libGTASA +  0x31EC0C))(NV_MULTITOUCH_MOVE, 1, x2, y2); // AND_TouchEvent
            }
        }
        if ((x3 || y3) || num == 2)
        {
            ...
            {
                ((void(*)(int, int, int posX, int posY))(g_libGTASA +  0x31EC0C))(type, 2, x3, y3); // AND_TouchEvent
            }
            else
            {
                ((void(*)(int, int, int posX, int posY))(g_libGTASA +  0x31EC0C))(NV_MULTITOUCH_MOVE, 2, x3, y3); // AND_TouchEvent
            }
        }
    }
...
void MultiTouch::initialize()
{
    ...
    CHook::Write(g_libGTASA + 0x851D38, &test_pointsArray);

    memset(test_pointersLibArray, 0, 999 * sizeof(int));
    CHook::Write(g_libGTASA + 0x8B5028, test_pointersLibArray);

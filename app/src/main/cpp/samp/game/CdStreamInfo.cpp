...
int32 CdStreamOpen(const char* lpFileName) {
    return CHook::CallFunction<int32_t>(g_libGTASA + 0x38AF9C, lpFileName);
}
...
eCdStreamStatus CdStreamSync(int32 streamId) {
    return CHook::CallFunction<eCdStreamStatus>(g_libGTASA + 0x38AE10, streamId);
}

eCdStreamStatus CdStreamGetStatus(int32 streamId) {
    return CHook::CallFunction<eCdStreamStatus>(g_libGTASA + 0x38ADA4, streamId);
}
...
bool CdStreamRead(int32 streamId, void* lpBuffer, uint32 offsetAndHandle, int32 sectorCount) {
    return CHook::CallFunction<bool>(g_libGTASA + 0x38ACB8, streamId, lpBuffer, offsetAndHandle, sectorCount);
}
